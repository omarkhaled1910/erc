...
import { approveTokensTx, requestTokensTx } from "@/app/actions/request-tokens"
...
const ApproveTokenModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
    const {
        register,
        handleSubmit,
        formState: { errors },
    ...
    const approveTokens = useMutation({
        mutationFn: async ({ amount }: RequestTokenFormData) => {
            ...
            const txHash = await approveTokensTx(amount, address)
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-4">
                        <div className="space-y-2 flex gap-2 flex-col">
                            <input
                                ...
                                disabled={approveTokens.isPending}
                            />
                            {errors.amount && (
                            ...
                            <p className="text-sm text-gray-500">
                                TO Be Approved Address : {address}
                            </p>
                        </div>
                        ...
                        <Button
                            ...
                            className="w-full"
                        >
                            {approveTokens.isPending ? "Approving..." : "Approve Tokens"}
                        </Button>
                    </form>
                </DialogDescription>
            </DialogContent>
